%% 逐步网格搜索 + 动态范围调整的最佳参数组合范围研究
% 当前思路 依据起点统计结果与敏感性分析结果，从敏感性较大的width和length开始进行网格搜索和评价，后续再进行结果的更改；
% 均分width和length后动态调成取值位置，范围大小及精度，期望结果0.01精度的系数
% 计算原则，R可以求均值，流量不可
...
%% 保存CSV.MAT文件
save_path = fullfile(output_basicpath, 'FIRST_search_result.csv');
writetable(T_result_sorted, save_path);
disp(['csv细化搜索结果已保存到: ' save_path]);

mat_path = fullfile(output_basicpath, ['nodesCount',nodename, '_',p 'T_result_sorted.mat']);
