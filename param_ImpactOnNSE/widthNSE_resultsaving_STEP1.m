%% %% 在指定漫流宽度的情况下，寻找可以获得较优NSE的管长和节点数的参数组合，的第一轮计算的数据储存test
...
results_mat_file = fullfile(output_basicpath, ...
    sprintf('widthNSE_Nodes%s_%s.mat', nodes_str, timestamp));
results_csv_file = fullfile(output_basicpath, ...
    sprintf('widthNSE_Nodes%s_%s.csv', nodes_str, timestamp));
