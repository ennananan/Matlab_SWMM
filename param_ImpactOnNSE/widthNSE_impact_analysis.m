%% 在指定漫流宽度的情况下，寻找可以获得较优NSE的管长和节点数的参数组合
...
nodesCount = [1,2,3,4,5,6,7,8]; % R的页数
ave_length = 112.4;
C_width = 2114;
node_T = 127;
...
% 形成参数组合矩阵
% 这里仅仅只是对漫流宽度的初始范围进行了计算，因此初始值的起点对图像影响非常大，未曾进行漫流宽度范围的动态调整计算
...
% 保存图形
saveas(gcf, fullfile(pngsave_path, 'Dynamic_Layout_Width_NSE.png'));
